    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.penilaians.store") }}" enctype="multipart/form-data">
            ...
            {{-- <div class="form-group">
                <label for="image">{{ trans('cruds.penilaian.fields.image') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.penilaian.fields.image_helper') }}</span>
            </div> --}}
            <div class="form-group">
                <label for="nama_siswa">{{ trans('cruds.penilaian.fields.nama_siswa') }}</label>
                <input class="form-control {{ $errors->has('nama_siswa') ? 'is-invalid' : '' }}" type="text" name="nama_siswa" id="nama_siswa" value="{{ old('nama_siswa', '') }}">
                @if($errors->has('nama_siswa'))
                    <div class="invalid-feedback">
                        {{ $errors->first('nama_siswa') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.penilaian.fields.nama_siswa_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    url: '{{ route('admin.penilaians.storeMedia') }}',
    maxFilesize: 2, // MB
    ...
    init: function () {
@if(isset($penilaian) && $penilaian->image)
      var file = {!! json_encode($penilaian->image) !!}
